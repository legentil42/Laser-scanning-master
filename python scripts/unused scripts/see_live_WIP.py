...
def save_3DPC_as_txt(L_3DPC,name = "3DPC.txt"):
    import os
    if os.path.exists(name):
        os.remove(name)

    with open(name, 'w') as f:
        for i in range(len(L_3DPC)):
            f.write("%s\n" % L_3DPC[i])

save_3DPC_as_txt(L_points,"real_scan.txt")
...
with open("real_scan.txt", "r") as f:
    good_points = []
    for item in f:
        for each_point in ast.literal_eval(item[:-1]):
            if each_point[-1] != 0:
                good_points.append(each_point)
