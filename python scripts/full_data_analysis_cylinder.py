...
def selection():
    ...
    with open("big_cyl.svg", 'r') as file:
...
def Homography_from_corners(Scan_pts,L_corners_ori, show = True):
    
    corners_ori = L_corners_ori
...
#%%
L_scan = import_my_3DPC(12,file_paths,show=True) #x : 154.9 = dir 1, y:199.2
#%%
def full_analysis_of_a_scan(i_PC_to_do,mask_res_factor,kept_z=8.5,kept_z_tol=1,
    ...
    original_cyl = import_my_3DPC_simple("cyl_svg_ori.txt",show = show)

    for i in range(len(original_cyl)):
        original_cyl[i][1] = -original_cyl[i][1]
    #import the ith scan and keep the top part
    L_SCAN = import_my_3DPC(i_PC_to_do,file_paths,show = show)
    L_SCAN += import_my_3DPC(i_PC_to_do+1,file_paths,show = show)
    ...
    L_corners_ori = [[15.592947, -943.512438, 8.5], [16.878486, -338.543494, 8.5], [484.234726, -972.511627, 8.5], [482.552889, -338.543494, 8.5]]

    L_corners_SCAN =[[-445.34014271557027, 72.50333093679056, 8.17197271815823], [-445.25219219123005, 233.03216059946857, 8.173084332661773], [-330.28956428629425, 64.50657012404768, 7.856383047970667], [-332.89723554134673, 234.63524852742637, 8.250374048830395]]
    
    #L_corners_SCAN = selection_4_corners(L_kept)

    H = Homography_from_corners(L_corners_SCAN,L_corners_ori, show = show)
    new_z = mean_of_zs(L_corners_SCAN) 
    L_warped = apply_homography_to_svg(original_cyl,H,new_z)
    


    for i_crack in range(1):
        
        print("crack number "+str(i_crack))

        if show:
            PC_view_of_these_lists([L_warped,L_kept])
        ...
        if show:
            ...
            PC_view_of_these_lists([My_Line_SCAN_full_traj,L_kept],
                                point_size=0.1)


    return True
    
full_analysis_of_a_scan(12,10,show = True)
# %%



L_SCAN = import_my_3DPC(6,file_paths,show = True)
L_SCAN_2 = import_my_3DPC(2,file_paths,show = True)

L_kept = keep_top_z(L_SCAN,8.5,1,show = True)
L_kept2 = keep_top_z(L_SCAN_2,8.5,1,show = True)
